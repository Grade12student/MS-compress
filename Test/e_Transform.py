...
# Applying 2D DCT on the input image
def apply_dct(image):
    return dct(dct(image, axis=0, norm='ortho'), axis=1, norm='ortho')

# Applying inverse 2D DCT on the coefficients
...
def encoder(loadmodel, input_path, refer_path, outputfolder):
    ...
    # Applying DCT to the input images
    im1_dct = apply_dct(im1)
    im2_dct = apply_dct(im2)
